...
const SignIn: FunctionComponent = (): JSX.Element => {
    ...
    const [ emailError, setEmailError ] = useState<{ display: boolean, message: string }>({
        display: false,
        message: ERRORS.INVALID_EMAIL,
    });

    const [ passwordError, setPasswordError ] = useState<{ display: boolean, message: string }>({
        display: false,
    ...
    return (
        <ContentContainer height='100vh'>
            ...
            <Form onSubmit={handleSubmit}>
                ...
                { signUpError.display && <span style={errorMessageStyles}>{signUpError.message}</span> }
