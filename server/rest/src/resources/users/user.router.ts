...
router
  .route('/auth/signup')
  .post(async (req: e.Request, res: e.Response, next: e.NextFunction) => {
    try {
      const {
        firstName,
        lastName,
      ...
      if (!(await userService.isValidPassword(password))) {
        return res
          .status(422)
          .json({ error: 'Password must be longer than 4 characters' });
      }
  ...
  .post(async (req: e.Request, res: e.Response, next: e.NextFunction) => {
    try {
      ...
      if (!(await userService.exists({ email }))) {
        return res
          .status(404)
          .json({ error: 'User with the provided email is not found' });
      }
      ...
      try {
        ...
        if (await userService.exists({ _id: userId })) {
          ...
          return res.status(200).json({
            user: {
              _id: user._id,
              ...(await userService.getSafeResponse(user)),
            },
          });
        } else {
          return res.status(404).json({ error: "User doesn't exist" });
