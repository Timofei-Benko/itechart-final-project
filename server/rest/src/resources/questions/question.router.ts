...
router
  .route('/questions')
  .get(async (req: e.Request, res: e.Response, next: e.NextFunction) => {
    try {
  ...
  .post(
    ...
    async (req: e.Request, res: e.Response, next: e.NextFunction) => {
      try {
        ...
        const question: IQuestion = await questionService.create(
          req.body.question
        );
  ...
  .get(
    ...
    async (req: e.Request, res: e.Response, next: e.NextFunction) => {
      try {
        ...
        if (order) {
          const questions = await questionService.getAllUserQuestions(
            { user: userId },
            order
          );
          return res.status(200).json({ questions });
        ...
        const questions: Array<IQuestion> =
          await questionService.getAllUserQuestions({ user: userId });
...
router
  ...
  .get(async (req: e.Request, res: e.Response, next: e.NextFunction) => {
    try {
      ...
      const question: IQuestion = await questionService.getOneById({
        _id: questionId,
      });
...
router
  .route('/users/:userId/questions/:questionId')
  .delete(
    validateSession,
    async (req: e.Request, res: e.Response, next: e.NextFunction) => {
      try {
        ...
        if (user._id.toString() !== question.user._id.toString()) {
          return res
            .status(403)
            .json({ error: "You can't delete questions posted not by you" });
        }
  ...
  .put(
    ...
    async (req: e.Request, res: e.Response, next: e.NextFunction) => {
      try {
        ...
        if (
          !(await questionService.answerExists({ _id: questionId }, answerId))
        ) {
          return res.status(404).json({ error: "Answer doesn't exist" });
        }
        if (
          scoreUpdateDirection &&
          (await questionService.checkIfUserVoted(userId, questionId, answerId))
        ) {
          return res
            .status(403)
            .json({ error: 'You have already voted for this answer' });
        }
        if (scoreUpdateDirection) {
          await questionService.updateAnswerScore(
            userId,
            questionId,
            answerId,
            scoreUpdateDirection
        ...
        if (isBest) {
          ...
          const question = await questionService.getOneById({
            _id: questionId,
          });
          if (user._id.toString() !== question.user._id.toString()) {
            return res
              .status(403)
              .json({ error: "Question wasn't posted by you" });
          }
