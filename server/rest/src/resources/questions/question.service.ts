...
const USER_FIELDS_TO_POPULATE =
  '_id firstName lastName username languages experience questionQty answerQty likedAnswerQty bestAnswerQty';
...
const getAll = async (order?: string): Promise<Array<IQuestion>> => {
  ...
  return Question.find().populate('user', USER_FIELDS_TO_POPULATE);
};
...
const answerExists = async (
  ...
  const answer: IAnswer | undefined = question.answers.find(
...
const getAllUserQuestions = async (
  ...
  return Question.find(filter).populate('user', USER_FIELDS_TO_POPULATE);
};
...
const checkIfUserVoted = async (
  ...
  const foundUser = answer.usersVoted.find(
    (id) => id.toString() === userId.toString()
  );
  return !!foundUser;
};

const setIsBestStatus = async (
  questionId: string,
  answerId: string,
  value: string
) => {
  ...
  const userId = question.answers.find(
    (answer) => answer._id.toString() === answerId.toString()
  ).user;
